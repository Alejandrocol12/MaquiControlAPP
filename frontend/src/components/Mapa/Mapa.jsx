...
// Fix Leaflet default icon bug con webpack/CRA
...
function Mapa() {
    ...
    const usarGPS = async (maq) => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                ...
                // Reverse geocoding gratuito con Nominatim
                ...
                } catch { /* usa coordenadas si falla */ }
    ...
    const sinUbicacion = maquinas.filter(m => !m.latitud || !m.longitud);
    const centro = conUbicacion.length
        ? [conUbicacion[0].latitud, conUbicacion[0].longitud]
        : [4.7110, -74.0721]; // Colombia por defecto
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    borderRight: '1px solid #eaf0f8', padding: '12px'
                }}>
                    {maquinas.map(m => (
                        <div key={m.id} style={{
                            background: '#fff', border: '1px solid #eaf0f8',
                    ...
                    {sinUbicacion.length > 0 && (
                        <p style={{ fontSize: '11px', color: '#9aa5b4', marginTop: '8px', textAlign: 'center' }}>
                            {sinUbicacion.length} máquina{sinUbicacion.length > 1 ? 's' : ''} sin ubicación — usa el GPS para registrarlas
                        </p>
                    )}
                </div>
